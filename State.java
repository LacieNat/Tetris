...
public class State {
	...
	public void makeMove(int move) {
        ...
        public void visualizeMove(int[] move) {
            visualizeMove(move[ORIENT], move[SLOT]);
        }
	...
	public void makeMove(int[] move) {
        ...
        public void visualizeMove(int orient, int slot) {
            int[][] fieldD = field.clone();
            int turnD = turn+1;
            int[] topD = top.clone();
            //height if the first column makes contact
            int height = topD[slot]-pBottom[nextPiece][orient][0];
            //for each column beyond the first in the piece
            for(int c = 1; c < pWidth[nextPiece][orient];c++) {
                    height = Math.max(height,topD[slot+c]-pBottom[nextPiece][orient][c]);
            }
...
//            }
            ...
            for(int i = 0; i < pWidth[nextPiece][orient]; i++) {
                    ...
                    for(int h = height+pBottom[nextPiece][orient][i]; h < height+pTop[nextPiece][orient][i]; h++) {
                            fieldD[h][i+slot] = turnD;
                    }
            }

            //adjust top
            for(int c = 0; c < pWidth[nextPiece][orient]; c++) {
                    topD[slot+c]=height+pTop[nextPiece][orient][c];
            }
            ...
            for(int r = height+pHeight[nextPiece][orient]-1; r >= height; r--) {
                    ...
                    if(full) {
                            ...
                            for(int c = 0; c < COLS; c++) {
                                    ...
                                    for(int i = r; i < topD[c]; i++) {
                                            fieldD[i][c] = fieldD[i+1][c];
                                    }
                                    //lower the top
                                    topD[c]--;
                                    while(topD[c]>=1 && fieldD[topD[c]-1][c]==0)	topD[c]--;
                            }
                    }
            }
        }
